...
# Load data
pittsburgh <- read_sf("pittsburgh_erap_red_evic.geojson")
...
plot1 <- ggplot(pittsburgh_clean, aes(x = percent_black, y = eviction.rate)) +
  geom_point() +
...
plot2 <- ggplot(pittsburgh_clean, aes(x = percent_asian, y = eviction.rate)) +
  geom_point() +
...
plot3 <- ggplot(pittsburgh_clean, aes(x = percent_latine, y = eviction.rate)) +
  geom_point() +
...
plot4 <- ggplot(pittsburgh_clean, aes(x = percent_other, y = eviction.rate)) +
  geom_point() +
...
plot5 <- ggplot(pittsburgh_clean, aes(y= grade, x = eviction.filing.rate, fill = grade)) +
  geom_bar(stat = "identity") +
  labs(title = "Eviction Filling Rate by Grade", x = "Grade", y = "Eviction Filling Rate")

# Plot 6: Box Plot of Eviction Rate by Race
chicago_long <- pittsburgh_clean |>
...
plot9 <- ggplot(pittsburgh_clean, aes(x = index_value, y = eviction.rate)) +
  geom_point() +
...
plot10 <- ggplot(pittsburgh_clean, aes(x = median.gross.rent, y = eviction.rate)) +
  geom_point() +
...
grid.arrange(
  ...
  top = textGrob("Pittsburg Graphs", gp = gpar(fontsize = 20, font = 2))
)
